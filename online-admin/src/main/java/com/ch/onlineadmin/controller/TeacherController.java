...
public class TeacherController {
    ...
    @ApiOperation("获取题目信息,可分页 ----> 查询条件(可无)(questionType,questionBank,questionContent,createPerson),必须有的(pageNo,pageSize)")
    public CommonResult<List<Question>> getQuestion(@RequestParam(required = false) String questionType,
        ...
        if (!Objects.equals(questionType, "") && questionType != null) wrapper.eq("qu_type", questionType);
        if (!Objects.equals(questionBank, "") && questionBank != null) wrapper.like("qu_bank_name", questionBank);
        if (!Objects.equals(questionContent, "") && questionContent != null) wrapper.like("qu_content", questionContent);
        if (!Objects.equals(createPerson, "") && createPerson != null) wrapper.eq("create_person", createPerson);
    ...
    public CommonResult<Integer> addExamRecord(@RequestBody ExamRecord examRecord, HttpServletRequest request) {
        ...
        System.out.println("保存考试记录，creditImgUrl: " + examRecord.getCreditImgUrl());
        examRecord.setExamTime(new Date());
        examRecordService.save(examRecord);
        log.info("考试记录保存成功，recordId: {}, creditImgUrl: {}", id, examRecord.getCreditImgUrl());
