  ...
  <el-container>
    ...
    <el-header height="220">
      ...
      <el-row style="margin-top: 10px">
        ...
        <el-checkbox v-model="onlyMyQuestions" @change="filterMyQuestions" style="margin-left: 15px;">
          只查看我添加的题目
        </el-checkbox>
      </el-row>
    </el-header>
    ...
    methods: {
      ...
      getCurrentUsername () {
        this.$http.get(this.API.checkToken).then((resp) => {
          if (resp.data.code === 200) {
            this.currentUsername = resp.data.data.username
          }
        })
      },
      //筛选自己添加的题目
      filterMyQuestions () {
        ...
        if (this.onlyMyQuestions) {
          this.queryInfo.createPerson = this.currentUsername
        } else {
          this.queryInfo.createPerson = ''
        }
        this.getQuestionInfo()
      },
      //获取题目信息
      getQuestionInfo () {
        // 如果选择了只查看自己的题目，设置createPerson
        if (this.onlyMyQuestions && this.currentUsername) {
          this.queryInfo.createPerson = this.currentUsername
        } else if (!this.onlyMyQuestions) {
          this.queryInfo.createPerson = ''
        }
