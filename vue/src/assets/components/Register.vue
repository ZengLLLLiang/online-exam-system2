        ...
        <div>
          <el-form :model="registerForm" :rules="registerFormRules" ref="registerForm" :status-icon="true"
            ...
            <el-form-item prop="username">
              <el-input prefix-icon="el-icon-user" v-model="registerForm.username" placeholder="账号" @input="handleUsernameInput"></el-input>
            </el-form-item>
  ...
  export default {
    ...
    data () {
      ...
      var validateUsername = (rule, value, callback) => {
        ...
        // 检查是否包含空格（去除首尾空格后，如果还有空格则说明中间有空格）
        ...
        this.$http.get(this.API.checkUsername + '/' + trimmedUsername).then((resp) => {
          ...
          callback(new Error('检查用户名时出错，请稍后重试'))
        })
      }
      return {
        //登录表单数据信息
    ...
    methods: {
      ...
      submitForm () {
        this.$refs['registerForm'].validate((valid) => {
          if (valid) {//验证通过
            // 确保提交前用户名已去除空格
            const trimmedUsername = this.registerForm.username ? this.registerForm.username.trim() : ''
            if (trimmedUsername !== this.registerForm.username) {
              this.registerForm.username = trimmedUsername
            }
      ...
      handleUsernameInput (value) {
        ...
        if (value) {
          ...
          // 只有当值发生变化时才更新，避免循环触发
          if (noSpaceValue !== this.registerForm.username) {
            this.registerForm.username = noSpaceValue
          }
        } else {
