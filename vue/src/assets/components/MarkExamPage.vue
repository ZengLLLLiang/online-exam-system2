<template>
  <el-container>
    ...
    <!--诚信考试图片-->
    <el-dialog :visible.sync="creditDialog" @close="creditDialog = false" title="诚信截图" width="80%">
      <div v-if="getCreditImages().length === 0" style="text-align: center; padding: 40px;">
        <p>该考试记录暂无诚信截图</p>
      </div>
      <div v-else style="display: flex; flex-wrap: wrap; gap: 20px;">
        <div v-for="(item, index) in getCreditImages()" :key="index" style="flex: 1; min-width: 300px;">
          <img :src="item" alt="诚信截图" style="width: 100%; height: auto; border: 1px solid #ddd; border-radius: 4px; cursor: pointer;" @click="showBigImg(item)">
          <p style="text-align: center; margin-top: 10px;">截图 {{ index + 1 }}</p>
        </div>
      </div>
    </el-dialog>
  </el-container>
...
<script>
  export default {
    ...
    methods: {
      ...
      //获取诚信截图列表
      getCreditImages () {
        if (!this.examRecord || !this.examRecord.creditImgUrl || 
            (typeof this.examRecord.creditImgUrl === 'string' && this.examRecord.creditImgUrl.trim() === '')) {
          return []
        }
        return this.examRecord.creditImgUrl.split(',').filter(url => url && url.trim() !== '')
      },
      //根据考试id查询考试中每一题的分数
