<template>
  ...
  <el-container v-if="show">
    ...
    <el-main>
      ...
      <video id="video" muted="muted" 
             style="float:right;position: fixed;top: 80%;left: 85%;width: 200px;height: 200px;background-color: #000;object-fit: cover;border: 2px solid #ddd;border-radius: 4px;" 
             autoplay="autoplay" playsinline webkit-playsinline></video>
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      if (this.duration > 1) {
        for (let i = 0; i < 2; i++) {
          let randomTime = Math.ceil(Math.random() * (this.duration - 1) * 1000) + 10000
      ...
      if (!isIE && !isEdge && !isIE11) {//兼容chrome和firefox
        ...
        window.onunload = function () {
          ...
          if (_gap_time <= 5) {
            localStorage.removeItem('examDuration' + self.examInfo.examId)
          } else {//谷歌浏览器刷新
          }
        }
    ...
    methods: {
      ...
      getCamera () {
        ...
        promise.then((mediaStream) => {
          // 获取视频轨道（video track通常是第一个，索引0）
          const videoTracks = mediaStream.getVideoTracks()
          if (videoTracks.length > 0) {
            this.mediaStreamTrack = videoTracks[0]
          } else {
            // 如果没有video tracks，尝试从所有tracks中获取
            const allTracks = mediaStream.getTracks()
            const videoTrack = allTracks.find(track => track.kind === 'video')
            if (videoTrack) {
              this.mediaStreamTrack = videoTrack
            }
          }
          ...
          this.cameraOn = false
          let errorMessage = '请开启摄像头权限'
          if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
            errorMessage = '摄像头权限被拒绝，请允许使用摄像头'
      ...
      async uploadExamToAdmin () {
        ...
        if (this.cameraOn) {
          try {
            ...
            console.error('最后一张照片上传失败:', error)
          }
        }
        
        // 等待所有正在上传的照片完成（给一个短暂的延迟确保上传完成）
        ...
        if (this.userAnswer.length < this.questionInfo.length) {
          this.$confirm('当前试题暂未做完, 是否继续提交o(╥﹏╥)o ?', 'Tips', {
            ...
            // 只有当有照片时才设置creditImgUrl
            if (this.takePhotoUrl.length > 0) {
              data.creditImgUrl = this.takePhotoUrl.join(',')
            } else {
              data.creditImgUrl = null
            }
            console.log('提交考试记录，诚信照片数量:', this.takePhotoUrl.length, 'URLs:', this.takePhotoUrl)
      ...
      duration (newVal) {
        ...
        if (newVal < 1) {
          ...
          (async () => {
            ...
            console.log('时间结束自动提交，诚信照片数量:', this.takePhotoUrl.length, 'URLs:', this.takePhotoUrl)
            this.$http.post(this.API.addExamRecord, data).then((resp) => {
              if (resp.data.code === 200) {
                this.$notify({
                  title: 'Tips',
